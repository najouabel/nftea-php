<?php 
   ...
   $query = $db->prepare('select * from collection ');
   $data = $query->fetch();
   if(isset($_POST['submit'])){
	$NameColl=$_POST['NameColl'];?>
  <?php $req = $db->query("SELECT * FROM `user`");
                        ...
                        <?php  
?> <?php
	$UserId= $data['UserId'] ;?><?php }endwhile; ?> <?php
	$req = $db->prepare("insert into collection (NameColl,UserId) values(?,?)");
	$req->execute([$NameColl,$UserId]);
	header('location:/nftea-site-/user/listcoll.php?msg=added');
   }
?>
<!DOCTYPE html>
<html lang="en">
...
?> <?php endwhile; ?> 
   ...
   <button> <a href="addnft.php">Create nft</a></button>
      <button><a href="addnft.php">Create collection</a></button>
						   ...
						   <div class="ajout-one">
							   ...
							   <h3> Create collection:</h3>
								   ...
								   <div class="ajout-tree">
									   ...
									   <input type="text" id="NameColl" name="NameColl"  placeholder="NameColl">
								   </div>
							   </div>
							   <div class="ajout-two">
