...
?> <?php endwhile; ?> 
    ...
    <?php 
                        ...
                        WHERE nft.CollectionId = collection.CollectionId" );
                        while($data =  $req->fetch()):
...
?>
    ...
    <img src="./img/<?= $data['imageNft'] ?>"  class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title"><?= $data['NameNft'] ?></h5>
        <p class="card-text"> <?= $data['description'] ?> <br><?= $data['Prix'] ?></p>
        <a href="delete.php?IdNft=<?= $data['IdNft'] ?>" class="btn btn-primary" onclick="return confirm('Do you want delete this record?');">Delete</a>
        <a href="update.php?IdNft=<?= $data['IdNft'] ?>" class="btn btn-primary" onclick="return <?= $data['IdNft'] ?>;">Update</a>
      </div>
    </div>
    <?php }endwhile; ?>
